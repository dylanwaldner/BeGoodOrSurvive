    ...
    if rank == 0:
        ...
        while loop_counter < max_loops:
            ...
            if train:
                ...
                losses.append((global_counter, loss.item()))
            ...
            if global_counter in [500, 1000, 1500, 3000]:
                break
        ...
        # Extract only the loss values (second item of each tuple)
        loss_values = [loss_tuple[1] for loss_tuple in losses]

        # Summarize losses
        loss = sum(loss_values) / len(loss_values) if loss_values else 0
        ...
        loss_summary = {
            ...
            "median_loss": float(np.median(loss_values)) if loss_values else 0.0,
            "std_loss": float(np.std(loss_values)) if loss_values else 0.0,
            "upper_quartile": float(np.percentile(loss_values, 75)) if loss_values else 0.0,
            "lower_quartile": float(np.percentile(loss_values, 25)) if loss_values else 0.0,
            "iqr_loss": float(np.percentile(loss_values, 75) - np.percentile(loss_values, 25)) if loss_values else 0.0,
            "top_5_losses": sorted(loss_values, reverse=True)[:5] if loss_values else [],
            "bottom_5_losses": sorted(loss_values)[:5] if loss_values else []
        }
...
def generational_driver(votes, max_tokens, temperature, top_p, danger, shared_history, bnn_history, ground_truth_label_list, ethical_ground_truths, gen_loss_history, gen_ethical_history, strong_bnn, config, num_gens, neat_trainer, global_counter, comm, genome):
    rank = comm.Get_rank()
    size = comm.Get_size()
    ...
    if rank == 0:
        checkpoint_path = "checkpoint_2025-02-03_16-10-11.pth"
        ...
        if rank == 0 and os.path.exists(checkpoint_path):
            try:
                ...
                if isinstance(bnn_history, list):
                    print(f"bnn_history loaded with {len(bnn_history)} entries.")
                else:
                    print(f"Warning: Unexpected format for bnn_history. Type: {type(bnn_history)}")
                    try:
                        list(bnn_history)
                    except Exception as e:
                        print(f"Could not convert to list, error = {e}")
                        bnn_history = []
                ...
                winner_genome_checkpoint_path = "newrun.pth"
                if os.path.exists(winner_genome_checkpoint_path):
                    try:
                        ...
                        # Validate required keys in winner genome checkpoint
                        winner_required_keys = ['genome', 'attention_layers', 'config', 'model_state_dict']
                        missing_winner_keys = [key for key in winner_required_keys if key not in winner_genome_checkpoint]
                        if missing_winner_keys:
                            print(f"Winner genome checkpoint is missing required keys: {missing_winner_keys}")
                        ...
                        print(f"Warning: Failed to load winner genome checkpoint. Error: {e}")
                        print("Using previous model state instead.")


                else:
                    print(f"Winner genome checkpoint not found at {winner_genome_checkpoint_path}. Using previous model state.")
                    attention_file_path = f"saved_genomes/neat_iteration_1/attention_layers_1.pth"
                    attention_file_data = torch.load(attention_file_path, map_location=torch.device("cpu"))
                    genome = genome
                    attention_layers = attention_file_data
                    config = config
                    strong_bnn = BayesianNN(genome, config, attention_layers=attention_layers)
            ...
            print(f"✅ Checkpoint Loaded! Global Counter: {global_counter} (Expected: 499)")
            print(f"✅ Pop Size in Config: {config.pop_size} (Expected: 128)")
            print(f"✅ BNN History Length: {len(bnn_history)} (Check if one was removed)")
            print(f"✅ NEAT Trainer State Exists: {hasattr(neat_trainer, 'get_state')}")
            print(f"✅ Attention Layers Loaded: {list(attention_layers.keys())}")
    ...
    while global_counter < train_samples:
        ...
        if global_counter in [500, 1000, 1500]:    
            ...
            winner_genome = neat_trainer.run_neat_step(strong_bnn, bnn_history, ground_truth_label_list, ethical_ground_truths, comm, attention_layers)
            ...
            if rank == 0:
                ...
                iteration_save_path = f"test4_prod_best_architecture_iteration_{neat_iteration}.txt"
                ...
                tradeoff_save_path = f'test4_prod_population_tradeoffs_iteration_{neat_iteration}.json'
                ...
                model_save_path = f"test4_prod_winner_genome_model_iteration_{neat_iteration}.pth"
                torch.save({
    ...
    with torch.no_grad():
        ...
        samples_per_danger = 150  # Fixed samples per danger level
    ...
    if rank == 0:
        ...
        try:
            with open("test4_prod_experiment_summary.json", "w") as summary_file:
                json.dump(overall_summary_serializable, summary_file, indent=4)
            print(f"Experiment summary saved to 'test4_prod_experiment_summary.json'")
        except Exception as e:
